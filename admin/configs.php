	...
	items:[
		...
		new Ext.form.FieldSet({
			...
			items:[
				new Ext.form.Checkbox({
					fieldLabel:"IP제한 활성화",
					name:"enable_security_database_mode",
					boxLabel:"관리자모드의 데이터베이스관리를 특정 IP에서만 접근할 수 있도록 설정합니다.",
					uncheckedValue:"off",
					listeners:{
						change:function(form,checked) {
							form.getForm().findField("allow_database_ip").setDisabled(!checked);
						}
					}
				}),
				new Ext.form.TextArea({
					fieldLabel:"접근허용IP",
					name:"allow_database_ip",
					disabled:true,
					afterBodyEl:'<div class="x-form-help">관리자모드 데이터베이스관리에 접근을 허용할 아이피를 줄바꿈으로 구분하여 입력하여 주십시오.<br>특정IP 또는 IP대역(123.123.123.* 또는 123.*.*.* 등)으로 입력하실 수 있습니다.</div>'
				})
			]
		})
	]
