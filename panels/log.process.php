			...
			new Ext.grid.Panel({
				...
				listeners:{
					...
					itemcontextmenu:function(grid,record,item,index,e) {
						...
						menu.add({
							iconCls:"xi xi-cog",
							text:record.data.action + " 검색",
							handler:function() {
								Ext.getCmp("LogKeyword").setValue(record.data.action);
								Ext.getCmp("LogList").getStore().getProxy().setExtraParam("keyword",record.data.action);
								Ext.getCmp("LogList").getStore().loadPage(1);
							}
						});
					...
					selectionchange:function(selection,selected) {
						if (selected.length == 0) {
							...
							$("#LogRequest").text(JSON.stringify(JSON.parse(data.params),undefined,4));
