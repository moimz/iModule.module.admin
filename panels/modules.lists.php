...
Ext.onReady(function () { Ext.getCmp("iModuleAdminPanel").add(
	new Ext.grid.Panel({
		...
		border:false,
		tbar:[
			Admin.searchField("ModuleKeyword",200,"모듈검색",function(keyword) {
				Ext.getCmp("ModuleList").getStore().clearFilter();
				
				if (keyword.length > 0) {
					Ext.getCmp("ModuleList").getStore().filter(function(record) {
